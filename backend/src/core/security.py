...
from models.jwt import JWTCreate, JWTInstance, JWTRefreshPersistence, JWTSingleToken
...
def decode_token(data: JWTSingleToken) -> JWTRefreshPersistence:
    raw_data = jwt.decode(data.token, config.JWT_KEY)
    return JWTRefreshPersistence(
